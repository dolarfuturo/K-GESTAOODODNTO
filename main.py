...
# 1. ESTILO DO PAINEL (MANTIDO EXATAMENTE IGUAL)
...
try:
    ...
    # PEGA O MODELO NA J2 (Coluna 10, Linha 1)
    modelo_da_gerente = str(df.iloc[0, 9]) 
    ...
    for index, row in df.iterrows():
        ...
        # O BOTÃO USA O TEXTO DA J2 AQUI:
        msg_final = modelo_da_gerente.replace("{nome}", nome).replace("{atraso}", v_atraso).replace("{entrada}", v_entrada).replace("{pix}", pix)
        ...
        with st.container():
            ...
            with c4:
                url = f"https://wa.me/{tel}?text={quote(msg_final)}"
